...
const sqlDependencyDir = path.resolve(
  __dirname,
  "node_modules",
  ".bin",
  "sqlcmd",
...
if (!process.env.MSSQL_PASSWORD)
  process.env.MSSQL_PASSWORD = global.getConfig().password;
if (!process.env.MSSQL_DATABASE)
  process.env.MSSQL_DATABASE = global.getConfig().database;

const catFileCmd = (isWin ? "type " : "cat ") + sqlFileDir;

const sqlcmd =
  `${catFileCmd} | ${sqlDependencyDir} -s ${process.env.MSSQL_HOST} -o ${process.env.MSSQL_PORT} ` +
  `-u "${process.env.MSSQL_USER}" -p "${process.env.MSSQL_PASSWORD}" -d "${process.env.MSSQL_DATABASE}"`;

exec(sqlcmd, function (err, _result) {
