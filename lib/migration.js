...
"use strict";
// const g = require('strong-globalize')(); // Unused here? or maybe implicitly used if errors are thrown.
...
function mixinMigration(MsSQL) {
  ...
  MsSQL.prototype.showIndexes = function (model, cb) {
    ...
    const sql =
      "SELECT OBJECT_SCHEMA_NAME(T.[object_id],DB_ID()) AS [table_schema]," +
      ...
      " AND OBJECT_SCHEMA_NAME(T.[object_id],DB_ID()) = " +
      schema +
      " AND T.[name] = " +
      table +
      ...
      return new Promise((resolve, reject) => {
        self.getTableStatus(model, function (err, fields, indexes) {
          ...
          self.alterTable(
            ...
            fields,
            indexes,
            function (err, needAlter) {
              if (err) return reject(err);
              if (needAlter) ok = true;
              resolve();
            },
            true,
          );
  ...
  MsSQL.prototype.getColumnsToDrop = function (model, actualFields) {
    ...
    if (actualFields) {
      ...
      if (columnsToDrop.length) {
        statements.push(
          "DROP COLUMN" + columnsToDrop.join("," + MsSQL.newline),
        );
      }
    }
    return statements;
  ...
  MsSQL.prototype.addIndexes = function (model, actualIndexes) {
    const self = this;
    const m = this._models[model];
    // const idName = this.idName(model); // Unused
    ...
    aiNames.forEach(function (indexName) {
      ...
      if (
        (indexNames.indexOf(indexName) === -1 && !m.properties[indexName]) ||
        ...
        if (indexNames.indexOf(indexName) !== -1) {
          m.settings.indexes[indexName].columns
            .split(/,\s*/)
            .forEach(function (columnName, i) {
        ...
        if (!orderMatched) {
          sql.push(
            "DROP INDEX " +
              self.columnEscaped(model, indexName) +
              " ON " +
    ...
    indexNames.forEach(function (indexName) {
      ...
      if (!found) {
        ...
        splitcolumns.forEach(function (elem) {
  ...
  MsSQL.prototype.alterTable = function (
    model,
    actualFields,
    actualIndexes,
    done,
    ...
    const applyChange = (query) => {
      return new Promise((resolve, reject) => {
        if (checkOnly) {
          resolve({ statements: statements, query: query });
        } else {
          self.applySqlChanges(model, [query], function (err) {
            if (err) reject(err);
            else resolve();
          });
        }
    ...
    // Use reduce for series execution
    statements
      .reduce((promise, query) => {
        return promise.then(() => applyChange(query));
      }, Promise.resolve())
      .then((result) => {
        if (checkOnly) {
  ...
  MsSQL.prototype.propertiesSQL = function (model) {
      ...
      if (modelPKIDs && modelPKIDs.indexOf(prop) !== -1) {
        ...
        if (idProp.type === Number) {
          if (idProp.generated !== false) {
            sql.push(
              self.columnEscaped(model, modelPKID) +
                " " +
                self.columnDataType(model, modelPKID) +
          ...
          if (idProp.generated !== false) {
            ...
            sql.push(
              self.columnEscaped(model, modelPKID) +
                " " +
                self.propertySettingsSQL(model, prop) +
                " DEFAULT newid()",
            );
          }
          continue;
    ...
    if (modelPKIDs && modelPKIDs.length) {
      ...
      cmd +=
        " " +
        modelPKIDs
          .map(function (modelPKID) {
            return self.columnEscaped(model, modelPKID) + " ASC";
          })
          .join(", ") +
        MsSQL.newline;
      cmd +=
  ...
  MsSQL.prototype.automigrate = function (models, cb) {
    ...
    const migrateModel = (model) => {
      return new Promise((resolve, reject) => {
        if (!(model in self._models)) {
          return reject(new Error(g.f("Model not found: %s", model)));
        }
    ...
    Promise.all(models.map(migrateModel))
      .then(() => cb(null))
      .catch((err) => cb(err));
  };
  ...
  MsSQL.prototype.createIndexes = function (model) {
    ...
    if (dxs) {
      Object.keys(this._models[model].settings.indexes).forEach(
        function (prop) {
  ...
  MsSQL.prototype.columnDataType = function (model, property) {
    ...
    const colLength =
      (columnMetadata && columnMetadata.dataLength) || prop.length;
    if (colType) {
