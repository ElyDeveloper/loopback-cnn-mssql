...
function mixinDiscovery(MsSQL) {
  MsSQL.prototype.paginateSQL = function (sql, orderBy, options) {
    options = options || {};
    let offset = options.offset || options.skip;
    if (isNaN(offset)) {
      offset = 0;
    }
    ...
    const select = sql.substring(0, index);
    const from = sql.substring(index);
    if (orderBy) {
    ...
    let paginatedSQL =
      ...
      from +
      MsSQL.newline;
    paginatedSQL += ") AS S" + MsSQL.newline + "WHERE S.rowNum > " + offset;
  ...
  MsSQL.prototype.buildQueryColumns = function (owner, table) {
    ...
    if (owner) {
      sql = this.paginateSQL(
        'SELECT TABLE_SCHEMA AS "owner", TABLE_NAME AS "tableName", COLUMN_NAME' +
      ...
      sql = this.paginateSQL(
        'SELECT schema_name() AS "owner", TABLE_NAME' +
   ...
   * @returns {string}
   */
  MsSQL.prototype.buildQueryExportedForeignKeys = function (
    owner,
    table,
    _options,
  ) {
  ...
  MsSQL.prototype.buildPropertyType = function (columnDefinition, _options) {
    const mysqlType = columnDefinition.dataType;
    // const dataLength = options.dataLength || columnDefinition.dataLength; // Unused
  ...
  MsSQL.prototype.setDefaultOptions = function (_options) {};

  MsSQL.prototype.setNullableProperty = function (_property) {};
