...
class MsSQL extends SqlConnector {
  constructor(settings) {
    ...
    // Create a new object for connection config to avoid mutating original settings
    // and to ensure we have full control over what is passed to the driver
    this.connConfig = { ...this.settings };
    
    // Map host/hostname to server
    this.connConfig.server =
      this.settings.host || this.settings.hostname || this.settings.server;
    this.connConfig.user = this.settings.user || this.settings.username;

    // Ensure options object exists
    this.connConfig.options = this.connConfig.options || {};

    // Move root-level SSL/tedious options to options object if present
    const rootOptions = [
      "encrypt",
    ...
    rootOptions.forEach((opt) => {
      if (this.settings[opt] !== undefined) {
        this.connConfig.options[opt] = this.settings[opt];
      }
    });
    ...
    if (this.settings.url) {
      ...
      try {
        ...
        // Apply URL settings ONLY if not explicitly provided in the config object
        // This allows explicit settings (like host with named instance) to take precedence
        if (!this.connConfig.server) this.connConfig.server = url.hostname;
        if (!this.connConfig.user) this.connConfig.user = url.username;
        if (!this.connConfig.password) this.connConfig.password = url.password;
        if (!this.connConfig.database)
          this.connConfig.database = url.pathname.replace(/^\//, "");
        if (!this.connConfig.port && url.port)
          this.connConfig.port = parseInt(url.port);

        // Parse query params and add to options or config
        url.searchParams.forEach((value, key) => {
          ...
          if (rootOptions.includes(key)) {
            this.connConfig.options[key] = val;
          } else {
            // Add other params to root config (or could be options too depending on driver)
            // But usually query params are for connection options
            this.connConfig[key] = val;
          }
        });
      } catch (err) {
        debug("Error parsing connection URL: %s", err.message);
      }
      // Remove url from connConfig so the driver doesn't try to parse it again
      delete this.connConfig.url;
    }

    // Ensure port is a number if present
    if (this.connConfig.port) {
        this.connConfig.port = Number(this.connConfig.port);
    }
  ...
  buildInsertDefaultValues(_model, _data, _options) {
...
function isBigInt(num) {
  if (num > 2147483647 && num <= 9223372036854775807n) return true;
  if (num < -2147483648 && num >= -9223372036854775808n) return true;
  return false;
}
...
function parameterizedSQL(sql) {
  ...
  while (true) {
    ...
    count++;
    sql =
      sql.substring(0, index) + ("@param" + count) + sql.substring(index + 3);
  }
  return sql;
}
