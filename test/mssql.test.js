...
describe("mssql connector", function () {
  ...
  it("should support boolean types with false value", function (done) {
    Post.create(
      { title: "T2", content: "C2", approved: false },
      function (err, p) {
        should.not.exists(err);
        post = p;
        Post.findById(p.id, function (err, p) {
  ...
  it("should single quote escape", function (done) {
    Post.create(
      { title: "T2", content: "C,D", approved: false },
      function (err, p) {
        should.not.exists(err);
        post = p;
        Post.findById(p.id, function (err, p) {
  ...
  it("should return the model instance for upsert", function (done) {
    Post.upsert(
      { id: post.id, title: "T2_new", content: "C2_new", approved: true },
      function (err, p) {
  ...
  it("should escape number values to defect SQL injection in find - test 2", function (done) {
