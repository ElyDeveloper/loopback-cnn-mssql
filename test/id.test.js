    ...
    ds.automigrate(function (err) {
      ...
      async.series(
        [
          ...
          function (callback) {
            Model.create({ name: "w1" }, callback);
          },
          function (callback) {
            Model.create({ name: "w2" }, callback);
          },
          function (callback) {
            Model.create({ name: "w3" }, callback);
          },
          function (callback) {
            Model.find({ order: "id asc" }, function (err, results) {
              assert(!err);
    ...
    ds.automigrate(function (err) {
      ...
      async.series(
        [
          ...
          function (callback) {
            Model.create({ id: 501, name: "w1" }, callback);
          },
          function (callback) {
            Model.find({ order: "id asc" }, function (err, results) {
              assert(!err);
    ...
    ds.automigrate(function (err) {
      ...
      async.series(
        [
          ...
          function (callback) {
            Model.create({ idOne: 1, idTwo: 2, name: "w1" }, callback);
          },
          function (callback) {
            ds.discoverPrimaryKeys(
              "COMPOSITE_KEY_TEST",
              function (err, models) {
    ...
    ds.automigrate(function (err) {
      ...
      async.series(
        [
          ...
          function (callback) {
            Model.create({ id: 962744456683738, name: "w1" }, callback);
          },
          function (callback) {
            Model.find({ order: "id asc" }, function (err, results) {
              assert(!err);
